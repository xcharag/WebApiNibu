...
public class StudentInterestImpl(IBaseCrud<StudentInterest> baseCrud, OracleDbContext db) : IStudentInterest
{
    ...
    public async Task<Result<PagedResult<StudentInterestReadDto>>> GetAllAsync(StudentInterestFilter filter, PaginationParams pagination, CancellationToken ct)
    {
    ...
    public async Task<Result<StudentInterestReadDto>> CreateAsync(StudentInterestCreateDto dto, CancellationToken ct)
    {
        ...
        if (!validation.IsSuccess) return Result<StudentInterestReadDto>.Failure(validation.Errors);
    ...
    private async Task<Result<bool>> ValidateForeignKeysAsync(int schoolStudentId, int interestActivitieId, CancellationToken ct)
    {
        ...
        if (!await db.InterestActivities.AnyAsync(i => i.Id == interestActivitieId, ct)) errors.Add($"InterestActivitieId ({interestActivitieId}) not found");
    ...
    private static IQueryable<StudentInterest> ApplyFilters(IQueryable<StudentInterest> query, StudentInterestFilter filter)
    {
        ...
        if (filter.InterestActivityId.HasValue) query = query.Where(x => x.IdInterestActivity == filter.InterestActivityId.Value);
    ...
    private static StudentInterestReadDto MapToReadDto(StudentInterest i) => new()
    {
        Id = i.Id, MomentSelected = (WebApiNibu.Data.Dto.Person.MomentSelected?)i.MomentSelected,
        Moment = i.Moment, SchoolStudentId = i.IdSchoolStudent, InterestActivitieId = i.IdInterestActivity
    };

    private static StudentInterest MapFromCreateDto(StudentInterestCreateDto dto) => new()
    {
        MomentSelected = dto.MomentSelected.HasValue ? (WebApiNibu.Data.Enum.MomentSelected)dto.MomentSelected.Value : WebApiNibu.Data.Enum.MomentSelected.App,
        Moment = dto.Moment, IdSchoolStudent = dto.SchoolStudentId, IdInterestActivity = dto.InterestActivitieId,
        Active = true, SchoolStudent = null!, InterestActivity = null!
    };

    private static void ApplyUpdateDto(StudentInterest t, StudentInterestUpdateDto dto)
    {
        if (dto.MomentSelected.HasValue) t.MomentSelected = (WebApiNibu.Data.Enum.MomentSelected)dto.MomentSelected.Value;
        t.Moment = dto.Moment; t.IdSchoolStudent = dto.SchoolStudentId; t.IdInterestActivity = dto.InterestActivitieId;
    }
}
