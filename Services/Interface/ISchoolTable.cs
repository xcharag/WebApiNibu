...
public interface ISchoolTable
{
    ...
    Task<Result<SchoolTableReadDto>> CreateAsync(CreateSchoolTableCommand command, CancellationToken ct = default);
    Task<Result> UpdateAsync(int id, UpdateSchoolTableCommand command, CancellationToken ct = default);
